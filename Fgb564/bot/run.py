...
class WebServer():
  ...
  def __init__(self):
    ...
    def get_room_users():
      ...
      try:
        # أولاً: محاولة القراءة من الكاش مباشرة (أسرع وأكثر موثوقية)
        cache_max_age = 8  # ثواني
        if (hasattr(bot_instance, 'cached_room_users') and
            bot_instance.cached_room_users and
            time.time() - getattr(bot_instance, 'cached_room_users_time', 0) < cache_max_age):
          users_data = bot_instance.cached_room_users
          print(f"📊 تم الحصول على {len(users_data)} مستخدم من الكاش")
          return jsonify({
              'success': True,
              'users': users_data,
        ...
        # ثانياً: إذا لم يكن الكاش متاحاً، طلب تحديث من البوت
        # حذف أي ملف استجابة قديم أولاً لتجنب التعارض
        if os.path.exists('temp_users_response.json'):
          try:
        ...
        # انتظار الاستجابة مع timeout أقصر
        timeout = 8
        start_time = time.time()

        while not os.path.exists('temp_users_response.json'):
          if time.time() - start_time > timeout:
            # إذا انتهى الوقت، ارجع الكاش القديم إن وجد
            if hasattr(bot_instance, 'cached_room_users') and bot_instance.cached_room_users:
              users_data = bot_instance.cached_room_users
              if os.path.exists('temp_get_users.txt'):
                try: os.remove('temp_get_users.txt')
                except: pass
              return jsonify({
                  'success': True,
                  'users': users_data,
            ...
            return jsonify({'error': 'timeout', 'users': []}), 408
          time.sleep(0.1)
        ...
        try:
          ...
          users_data = getattr(bot_instance, 'cached_room_users', [])
        ...
        # محاولة إرجاع الكاش في حال الخطأ
        if hasattr(bot_instance, 'cached_room_users') and bot_instance.cached_room_users:
          return jsonify({
              'success': True,
              'users': bot_instance.cached_room_users,
              'count': len(bot_instance.cached_room_users),
...
class RunBot():
  ...
  def run_loop(self) -> None:
    ...
    # حلقة لا تنتهي — البوت يعيد الاتصال دائماً مهما حدث
    while True:
      attempt += 1
      try:
        ...
        print(f"🔄 محاولة الاتصال بـ Highrise... (المحاولة {attempt})")
        print(f"🏠 الغرفة المستهدفة: {self.room_id}")
        arun(main(self.definitions))

        # الاتصال انتهى بشكل طبيعي (مثلاً الغرفة فارغة أو قطع السيرفر)
        print("⚠️ انقطع الاتصال — إعادة الاتصال خلال 10 ثوانِ...")
        with open('bot_status.txt', 'w', encoding='utf-8') as f:
          f.write(f"DISCONNECTED:{time.time()}\n")
          f.write("REASON:NORMAL_DISCONNECT\n")
        time.sleep(10)
        ...
        if "multilogin" in error_str:
          sleep_time = 90
          print(f"⚠️ خطأ Multilogin - انتظار {sleep_time} ثانية (يوجد نسخة أخرى تعمل)...")
        elif "transport" in error_str or "connection" in error_str:
          sleep_time = 15
          print(f"🔄 مشكلة اتصال - إعادة المحاولة خلال {sleep_time} ثانية...")
        else:
          import traceback
          traceback.print_exc()
          sleep_time = 10
          print(f"❌ خطأ غير متوقع - إعادة المحاولة خلال {sleep_time} ثانية...")
